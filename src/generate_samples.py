...
def generate_samples(generators, noise_maps, reals, noise_amplitudes, config: ModelConfig, input_image=None, scale_v=1.0, scale_h=1.0,
    ...
    for G, Z_opt, noise_amp in zip(generators, noise_maps, noise_amplitudes):
        ...
        for n in tqdm(range(0, num_samples, 1)):
            ...
            # Get noise image
            z_curr = generate_spatial_noise([1, len(config.token_list), int(round(noise_size_x)), int(round(noise_size_y))], device=config.device)
            ...
            previous_image = interpolate(previous_image, [int(round(noise_size_x)), int(round(noise_size_y))], mode='bilinear', align_corners=False)
            ...
            if current_scale == len(reals)-1:
                dir2save = config.output_path + '/' + save_dir
                ...
                level = one_hot_environment_to_tokens(current_image.detach())
    ...
    ss = one_hot_environment_to_tokens(current_image[0].detach())
    ss = ss.cpu()
    import matplotlib.pyplot as plt
    plt.imshow(ss)
    plt.show()

    return current_image.detach()  # return last generated image (usually unused)